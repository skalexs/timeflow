  ...
  return (
    <div style={{ position: 'relative' }}>
          ...
          {value.map(tag => (
            <span key={tag.id} style={{
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 13, lineHeight: 1, padding: 0, marginLeft: 2 }}
              >×</button>
            </span>
          ))}
        </div>
      ...
      <button
        ...
        style={{
          background: 'var(--surface-2)', border: '1px dashed var(--border-2)', borderRadius: 8,
          color: 'var(--text-dim)', fontSize: 12, padding: '6px 12px', cursor: 'pointer', width: '100%',
          textAlign: 'left',
        }}
      >
        + Añadir tag
      ...
      {showPicker && (
        <div style={{
          ...
          <div style={{ marginBottom: 10, paddingBottom: 10, borderBottom: '1px solid var(--border)' }}>
            <div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase' }}>Crear nuevo</div>
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <button onClick={createTag} disabled={!input.trim() || loading}
                style={{ background: 'var(--accent)', border: 'none', borderRadius: 6, color: 'white', fontSize: 12, padding: '6px 10px', cursor: 'pointer', fontWeight: 600 }}>
                +
              </button>
            </div>
            {/* Color palette */}
              ...
              {PALETTE.map(c => (
                <button key={c} type="button"
                  ...
                  style={{
                    ...
                    cursor: 'pointer', outline: newColor === c ? '1px solid var(--accent)' : 'none',
                  }}
                />
              ))}
            </div>
          ...
          {unselected.length > 0 && (
            <div>
              <div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase' }}>Existentes</div>
