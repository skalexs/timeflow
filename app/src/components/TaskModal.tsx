...
export default function TaskModal({ isOpen, onClose, onSave, onDelete, initialTask, mode }: TaskModalProps) {
  const [form, setForm] = useState({ title: '', startTime: '09:00', endTime: '10:00', color: '#E54747', iconId: '📋' })
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-glass modal-spring" style={{ width: '100%', maxWidth: '420px', boxShadow: '0 24px 80px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text)', margin: 0 }}>{mode === 'create' ? 'Nueva tarea' : 'Editar tarea'}</h2>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            <label style={{ fontSize: '12px', color: 'var(--text-dim)', display: 'block', marginBottom: '6px' }}>Título</label>
            <input ref={inputRef} value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="¿Qué vas a hacer?" style={{ width: '100%', padding: '10px 12px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div>
            <label style={{ fontSize: '12px', color: 'var(--text-dim)', display: 'block', marginBottom: '8px' }}>Fecha</label>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '8px' }}>
              {[
                { key: 'today' as DatePreset, label: 'Hoy' },
                { key: 'tomorrow' as DatePreset, label: 'Mañana' },
                { key: 'evening' as DatePreset, label: 'Tarde' },
                { key: 'weekend' as DatePreset, label: 'Finde' },
                { key: 'nextWeek' as DatePreset, label: 'Semana' },
              ].map(preset => (
                <button
                  key={preset.key}
                  type="button"
                  ...
                  style={{
                    padding: '6px 12px',
                    borderRadius: '16px',
                    border: datePreset === preset.key ? '2px solid var(--accent)' : '1px solid var(--border)',
                    background: datePreset === preset.key ? 'var(--accent-soft)' : 'var(--surface)',
                    color: datePreset === preset.key ? 'var(--accent)' : 'var(--text-dim)',
                    fontSize: '12px',
              ...
              <button
                ...
                style={{
                  padding: '6px 12px',
                  borderRadius: '16px',
                  border: datePreset === 'custom' ? '2px solid var(--accent)' : '1px solid var(--border)',
                  background: datePreset === 'custom' ? 'var(--accent-soft)' : 'var(--surface)',
                  color: datePreset === 'custom' ? 'var(--accent)' : 'var(--text-dim)',
                  fontSize: '12px',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {(['startTime', 'endTime'] as const).map(key => (
              <div key={key}>
                <label style={{ fontSize: '12px', color: 'var(--text-dim)', display: 'block', marginBottom: '6px' }}>{key === 'startTime' ? 'Hora inicio' : 'Hora fin'}</label>
                <input type="time" value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} style={{ width: '100%', padding: '10px 12px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
            ))}
          </div>
          <div>
            <label style={{ fontSize: '12px', color: 'var(--text-dim)', display: 'block', marginBottom: '8px' }}>Color</label>
          ...
          <div>
            <label style={{ fontSize: '12px', color: 'var(--text-dim)', display: 'block', marginBottom: '8px' }}>Icono</label>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {ICONS.map(icon => (
                <button type="button" key={icon} onClick={() => setForm(f => ({ ...f, iconId: icon }))} style={{ width: '36px', height: '36px', borderRadius: '8px', background: form.iconId === icon ? 'var(--surface)' : 'var(--bg)', border: form.iconId === icon ? '2px solid var(--accent)' : '1px solid var(--border)', fontSize: '18px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>{icon}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
            ...
            <button type="button" onClick={onClose} style={{ padding: '10px 16px', borderRadius: '8px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-dim)', fontSize: '13px', cursor: 'pointer' }}>Cancelar</button>
            <button type="submit" disabled={saving} style={{ flex: 1, padding: '10px 16px', borderRadius: '8px', border: 'none', background: 'var(--accent)', color: 'white', fontSize: '13px', fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? 'Guardando...' : mode === 'create' ? 'Crear tarea' : 'Guardar cambios'}</button>
          </div>
        </form>
      </div>
    </div>
